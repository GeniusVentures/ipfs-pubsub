...
namespace
{
    ...
    int GetIPPriority( const std::string &ip )
    {
        ...
        if ( result != 1 )
        {
            return 999; // Invalid IP gets lowest priority
        }
    ...
    auto MakeCustomHostInjector( std::optional<libp2p::crypto::KeyPair> keyPair, Ts &&...args )
    {
        ...
        std::shared_ptr<crypto::CryptoProvider> crypto_provider = std::make_shared<crypto::CryptoProviderImpl>(
            csprng,
            ed25519_provider,
            rsa_provider,
            ecdsa_provider,
        ...
        auto injector = injector::makeHostInjector<di::extension::shared_config>(
            ...
            di::bind<crypto::marshaller::KeyMarshaller>()
...
namespace sgns::ipfs_pubsub
{
    ...
    void GossipPubSub::Init( std::optional<libp2p::crypto::KeyPair> keyPair )
    {
        ...
        protocol_config.autonat_callback = [this]() { this->StartProvidingCID(); };

        auto protocols = libp2p::protocol::factory::ProtocolFactory::createProtocols( m_host,
                                                                                      protocol_config,
                                                                                      injector );
                                                      ...
                                                      const std::vector<std::string> &addAddresses )
    {
        ...
        if ( !peerInfo )
        {
            auto errorMessage = ( boost::format( "%s: Cannot resolve local peer from the address" ) % m_localAddress )
                                    .str();
    ...
    void GossipPubSub::ScheduleNextFind( const libp2p::protocol::kademlia::ContentId &cid,
        ...
        MessageCallback    onMessageCallback )
    {
        ...
        auto subsF = [subscription, this, topic, onMessageCallback]()
        {
            ...
            {
                m_logger->info(
                    ( boost::format( "%s: PubSub subscribed to topic '%s'" ) % m_localAddress % topic ).str() );
            }
        };
                                                          ...
                                                          const std::vector<uint8_t> &message )
    {
        ...
        m_strand->post( [publish = std::move( publish ), completion]() mutable { completion->set_value( publish() ); } );
        return future.get();
    }
    ...
    void GossipPubSub::flushPendingMessages()
    {
        ...
        {
            m_logger->debug(
                ( boost::format( "%s: Flushed %d batched messages" ) % m_localAddress % messages_to_send.size() )
                    .str() );
        }
    }
    ...
    void GossipPubSub::refreshLocalAddresses()
    {
        ...
        try
        {
            ...
            {
                auto ma_str = ( boost::format( "/ip4/%s/tcp/%d/p2p/%s" ) % interface_ip % listening_port %
                                m_host->getId().toBase58() )
                                  .str();
            ...
            if ( !current_addresses.empty() )
            {
                ...
                if ( result )
                {
                    m_logger->info( "Successfully refreshed {} addresses with 10-minute TTL",
                                    current_addresses.size() );
                }
                else
                {
    ...
    void GossipPubSub::scheduleNextAddressRefresh()
    {
        ...
        m_address_monitor_timer->async_wait(
            [this]( const boost::system::error_code &ec )
            {
                if ( !ec && !m_context->stopped() )
                {
    ...
    void GossipPubSub::logCurrentNetworkState()
    {
        ...
        for ( const auto &local_addr : all_local_addrs )
        {
            ...
            auto activated_observed_for_addr = obs_repo.getAddressesFor( local_addr,
                                                                         true ); // Get only activated and confirmed
            m_logger->info( "Local address: {} -> {} raw observed, {} activated+confirmed",
                            local_addr.getStringAddress(),
                            raw_observed_for_addr.size(),
                            activated_observed_for_addr.size() );
        ...
        m_logger->info( "TOTAL: {} raw observed addresses, {} activated+confirmed addresses",
                        total_raw_observed.size(),
                        total_activated_observed.size() );
        ...
        m_logger->info( "NOTE: Current connections: {}, so max possible observers: {}",
                        connections.size(),
                        connections.size() );
